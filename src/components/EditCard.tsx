...
function EditCard({ id, onDelete, onDrag, onDrop }) {
    // State variables to manage editing and content for both sections(I Think The Initial Text entry bug is here?)
    const [isEditingA, setIsEditingA] = useState(false);
    const [isEditingB, setIsEditingB] = useState(false);
    const [contentA, setContentA] = useState(''); // Should be Set to Start with Data from Sets Cards
    const [contentB, setContentB] = useState(''); // Should be Set to Start with Data from Sets Cards
    ...
    const handleContentChangeA = (event) => {
        setContentA(event.target.value); // Use innerText to get the text content
    };

    // Function to update the content of section B(Answer)
    const handleContentChangeB = (event) => {
        setContentB(event.target.value); // Use innerText to get the text content
    };
    // JSX representing the card
    return (
        <div
            className={`editCard m-10`}
            draggable="true"
        ...
        >            {/* Top bar with delete and drag handle buttons */}
            ...
            <div className="content flex ">
                ...
                <div className="flex-grow">
                    ...
                    <button className=' m-3 p-3' type="button" onClick={handleButtonClickA}>
                        {isEditingA ? (
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
                ...
                <div className="flex-grow">
                    ...
                    <button className=' m-3 p-3' type="button" onClick={handleButtonClickB}>
                        {isEditingB ? (
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
